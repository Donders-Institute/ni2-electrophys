...
cov(x, y)=  (∑*i^n▒(x_i-x ̅ )(y_i-y ̅ ) )/(n-1)
...
> Verify remark 1. Hint 1: generate a data matrix, e.g. by using the command x=randn(10,100), and compute the covariance from this matrix. Hint 2: note that ‘mirroring across the main diagonal’ is the same as taking the matrix transpose.
...
> Use the mathematical definition of the filter output and derive the equation for the variance of the filter output. Hint: the variance of a vector (after subtracting the mean) is `a*a'`.
